...
const CampaignAdminShow = ({ role }: { role: string }) => {
  ...
  const getStatusStyle = (status: string) => {
    ...
    return statusStyles[status] || "bg-gray-100 text-gray-500 border-gray-200";
  };
  ...
  const handleApply = () => {
    window.location.href =`/candidate/c/${campaign?.publication_link}`
  };
    ...
    return (
      <div className="min-h-screen bg-base-200/50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to={role === 'candidate' ? '/campaigns' : '/admin/campaigns'} className="btn btn-primary">
            Retour à la liste
          </Link>
        </div>
      </div>
  ...
  const backLink =  isAdmin ? '/admin/campaigns' : '/candidate/campaigns'
  return (
    <div className="min-h-screen bg-base-200/50">
      <div className="bg-base-100 border-b border-base-300">
        <div className={`container mx-auto px-4 py-6 ${isCandidate ? 'pt-25' : ''}`}>
          <div className="flex items-center gap-4 mb-4">
            <Link 
              to={isCandidate ? '/campaigns' : '/admin/campaigns'} 
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
           <div className="card bg-base-100 shadow-sm border border-base-300">
            ...
            {/* Lien de publication - Visible uniquement pour les admins */}
            {isAdmin && campaign.publication_link && (
              <div className="card bg-base-100 shadow-sm border border-base-300">
                <div className="card-body">
                  <h2 className="card-title text-lg mb-4">
            ...
            <div className="sticky top-6 space-y-6">
              ...
              <div className="card bg-base-100 shadow-sm border border-base-300">
                <div className="card-body">
                  {!isAdmin && (
                    <>
                      {new Date(campaign.opening_date!) <= new Date() && daysRemaining > 0 && campaign.status === "open" ? (
                        <button 
                          onClick={handleApply}
                        ...
                        <div className="alert alert-warning">
                          <AlertCircle className="w-5 h-5" />
                          <span>La période de dépôt n’a pas encore commencé</span>
                        </div>
                      )}
       ...
       {isAdmin && <CandidateApplicationsTable />}
    </div>
  );
};
